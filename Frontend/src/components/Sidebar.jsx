...
const Sidebar = ({ isOpen, toggleSidebar }) => {
  ...
  // ✅ 1. Wrap fetchChats in useCallback to memorize the function
  ...
  // ✅ 2. Add empty dependency array []
  // This ensures it runs ONLY when the component first mounts.
  useEffect(() => {
    fetchChats();
  }, [fetchChats]); 
  ...
  const handleNewChat = async () => {
    try {
      ...
      if (response.status === 201 || response.status === 200) {
        // ✅ 3. Manually call fetchChats here (Only runs on successful create)
        await fetchChats(); 
      }
    } catch (error) {
      console.error("Error creating new chat:", error);
  ...
  const handleDeleteChat = async (e, chatIdToDelete) => {
    ...
    try {
      ...
      if (response.status === 204) {
        // ✅ 4. Manually call fetchChats here (Only runs on successful delete)
        await fetchChats();
